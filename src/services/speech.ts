...
function resolveClozeForSpeech(source: string, reveal: boolean): string {
  return String(source ?? "").replace(
    ...
    (_match, innerValue: string) => {
      ...
      const answer =
        separatorIndex >= 0 ? innerValue.slice(0, separatorIndex) : innerValue;
      const hint =
        separatorIndex >= 0 ? innerValue.slice(separatorIndex + 2).trim() : "";
...
function renderedContentToText(source: string): string {
  ...
  const html = renderContent(withoutFencedCode, false, true).replace(
    /<(?:br\s*\/?|\/(?:p|div|li|h[1-6]|blockquote|pre))>/gi,
    " ",
  );
  ...
  container
    .querySelectorAll("pre, audio, video, source")
